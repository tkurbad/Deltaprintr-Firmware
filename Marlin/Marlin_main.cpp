...
float FSR_TRIGGER_ADJUST=6.5;  // At minimum, there should be an increase of 0.8 degrees to trigger, changeable
float FSR_RECOVERY_ADJUST=4;   // value to bump up or down the threshold for recovery by.
...
bool touching_print_surface() {
  ...
  if ( FSR_READING >  ( FSR_RECOVERY_THRESHOLD ) ) {
    FSR_TRIGGERED=false;
...
void fsr_calibration() {
  // Calibrate the FSR sensor according to the bed temp reading. 
  ...
  int fsr_tally;
  float fsr_noise_max,fsr_noise_avg;
  float old_feedrate;
  int shake_idx;
...
void process_commands()
{
  ...
  else if(code_seen('M'))
  {
    ...
    case 777: // M777 fsr calibration use commands/options
      ...
      if( code_seen('C')) {
        // Recalibration requested for resting mode.
        SERIAL_ECHOPGM("Recalibrating FSR...");
        fsr_calibration();
        SERIAL_ECHOLNPGM("DONE.");
      };
      
      if( code_seen('T')){
        // Adjust the Trigger Level Adjustment Threshold
        FSR_TRIGGER_ADJUST = code_value();
      };
      
      if( code_seen('R')){
        // Adjust the Recovery Level Adjustment Threshold
        FSR_RECOVERY_ADJUST = code_value();
      };

      // Recalculate the thresholds
      fsr_recalculate();

      // Let's output a status update
      SERIAL_ECHOPGM("FSR:");
      FSR_TRIGGERED = touching_print_surface(); // Let's check to see if the surface is being touched.
      if ( FSR_TRIGGERED ) {
        SERIAL_ECHOLNPGM(" Triggered");
      } else {
        SERIAL_ECHOLNPGM(" Not Triggered");
      };
      ...
      SERIAL_ECHOPGM(" RestingAvg(R):");
      SERIAL_ECHO(fsr_resting);
      SERIAL_ECHOPGM(" RestingNoise:");
      SERIAL_ECHO(fsr_resting_noise);

      SERIAL_ECHOLNPGM(" ");

      SERIAL_ECHOPGM(" MovingAvg(M):");
      SERIAL_ECHO(fsr_moving);
      SERIAL_ECHOPGM(" MovingNoise:");
      SERIAL_ECHO(fsr_moving_noise);

      SERIAL_ECHOLNPGM(" ");

      SERIAL_ECHOPGM(" TriggerLevel:");
      SERIAL_ECHO(FSR_TRIGGER_THRESHOLD);
      SERIAL_ECHOPGM("/");
      SERIAL_ECHO(FSR_TRIGGER_THRESHOLD + FSR_TRIGGER_ADJUST);
      SERIAL_ECHOPGM(" RecoveryLevel:");
      SERIAL_ECHO(FSR_RECOVERY_THRESHOLD);
      SERIAL_ECHOPGM("/");
      SERIAL_ECHO(FSR_RECOVERY_THRESHOLD + FSR_RECOVERY_ADJUST);

      SERIAL_ECHOLNPGM(" ");

      SERIAL_ECHOPGM(" TriggerLevelAdjustment(T):");
      SERIAL_ECHO(FSR_TRIGGER_ADJUST);
      SERIAL_ECHOPGM(" RecoveryLevelAdjustment(R):");
      SERIAL_ECHO(FSR_RECOVERY_ADJUST);
